...
public class CharacterNetworkManager : NetworkBehaviour
{
    ...
    [Header("Animator")] public NetworkVariable <float> verticalMovement = 
        new NetworkVariable<float>(0, NetworkVariableReadPermission.Everyone, NetworkVariableWritePermission.Owner);
    [Header("Animator")] public NetworkVariable <float> moveAmount = 
        new NetworkVariable<float>(0, NetworkVariableReadPermission.Everyone, NetworkVariableWritePermission.Owner);
    ...
    // A SERVER RPC IS A FUNCTION CALLED FROM A CLIENT TO THE SERVER(IN THIS CASE, OUR HOST IS THE SERVER)
    [ServerRpc]
    public void NotifyTheServerOfActionAnimationServerRpc(ulong clientID, string animationID, bool applyRootMotion)
    {
        //IF THE PLAYER IS THE HOST/SERVER, THEN ACTIVATE CLIENT RPC
        if (IsServer)
        {
    ...
    //CLIENT RPC IS SENT TO ALL CLIENTS PRESENT BY THE SERVER
    [ClientRpc]
    public void PlayActionAnimationForAllClientsClientRpc(ulong clientID, string animationID, bool applyRootMotion)
    {
        // WE MAKE SURE WE DONT RUN THE FUNCTION ON THE CHARACTER WHO SENT IT (SO WE DONT PLAY THE ANIMATION TWICE)
